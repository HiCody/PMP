...
- (UIButton *)previewBtn{
    if (!_previewBtn) {
        ...
        [previewBtn setTitleColor:[UIColor colorWithRed:56/255.0 green:193/255.0 blue:255/255.0 alpha:1.0] forState:UIControlStateNormal];
...
- (void)editPic{
    ...
    MLSelectPhotoAssets *assets = self.selectAssets.firstObject;
    drawerVC.doodleImage = assets.originImage;
    __weak typeof(self)weakself = self;
    drawerVC.passImage = ^(UIImage *image){
        [weakself.selectAssets replaceObjectAtIndex:0 withObject:image];
        [self done];
...
- (void)preview{
    ...
    browserVc.maxCount = self.selectAssets.count;
...
- (void) setupToorBar{
    ...
    toorBar.items = @[leftItem,editItem,fiexItem,rightItem];
...
- (void) pickerCollectionViewDidSelected:(MLSelectPhotoPickerCollectionView *) pickerCollectionView deleteAsset:(MLSelectPhotoAssets *)deleteAssets{
    ...
    if (self.selectPickerAssets.count || deleteAssets) {
        ...
        for (NSInteger i = 0; i < self.selectAssets.count; i++) {
            if ([self.selectAssets[i] isMemberOfClass:[MLSelectPhotoAssets class]]) {
        ...
        self.editBtn.enabled = (self.selectAssets.count==1);
