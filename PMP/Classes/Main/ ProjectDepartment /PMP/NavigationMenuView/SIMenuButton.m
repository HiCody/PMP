...
- (void)layoutSubviews
{
   // [self.title sizeToFit];
    ...
    CGSize size=[self sizeWithText:self.title.text font:self.title.font maxSize:CGSizeMake(W(200), MAXFLOAT)];
    self.title.center = CGPointMake(self.frame.size.width/2, (self.frame.size.height-2.0)/2);
    //self.title.width = size.width;
    self.arrow.center = CGPointMake((CGRectGetMaxX(self.title.frame)-size.width)/2.0+size.width+2 + [SIMenuConfiguration arrowPadding], self.frame.size.height / 2);
}
