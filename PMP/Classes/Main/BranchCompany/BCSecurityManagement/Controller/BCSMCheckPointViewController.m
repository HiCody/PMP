//
//  BCSMCheckPointViewController.m
//  PMP
//
...
@interface BCSMCheckPointViewController ()<UITableViewDataSource,UITableViewDelegate,UIAlertViewDelegate>{
    NSInteger page;
    NSInteger total;
}
...
@implementation BCSMCheckPointViewController
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    NSArray *arr=[self.userdefaults objectForKey:kSubPosition];
    for (int i =0; i<arr.count; i++) {
        BCSMCheckPointModel *checkpoint =[[BCSMCheckPointModel alloc] init];
    ...
    if (alertView.tag==100) {
        if (buttonIndex==1) {
            ...
            if (!field.text.length||[field.text isEqualToString:@""]) {
                ...
                [self.dataSourcelist enumerateObjectsUsingBlock:^(BCSMCheckPointModel *obj, NSUInteger idx, BOOL * stop) {
                ...
                if (num==-1) {
                    NSInteger count=-1;
                    NSArray *tempArr=[self.userdefaults objectForKey:kSubPosition];
                    ...
                    if (count==-1) {
                        ...
                        [self.userdefaults setObject:arr forKey:kSubPosition];
                    ...
                    BCSMCheckPointModel *checkpoint =self.dataSourcelist[num];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==0) {
        if (self.saveDatalist.count==0) {
            ...
            BCSMCheckPointModel *checkpoint = self.saveDatalist[indexPath.row];
        ...
        if (self.dataSourcelist.count ==0) {
            ...
            BCSMCheckPointModel *checkpoint = self.dataSourcelist[indexPath.row];
            cell.textLabel.text=checkpoint.positionName;
            cell.textLabel.font =[UIFont  systemFontOfSize:15.0];
            cell.textLabel.textColor =[UIColor blackColor];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        if (indexPath.section==0&&self.saveDatalist.count){
            ...
            NSMutableArray *arr=[[self.userdefaults objectForKey:kSubPosition] mutableCopy];
...
- (void)queryCheckPositionRequset{
    ...
    [NetRequestClass NetRequestPOSTWithRequestURL:kQueryCheckPosition WithParameter:parameters WithReturnValeuBlock:^(id returnValue) {
        ...
        NSArray *tempArr=[BCSMCheckPointModel mj_objectArrayWithKeyValuesArray:dict[@"rows"]];
